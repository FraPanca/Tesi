...
Contiene le unit systemd per l'avvio automatico del sistema e per un job pianificato:
...
Le prime due sono unit distinte, non una sola, perché hanno cicli di vita diversi (containerizzato o processo diretto sull'host) e una dipendenza d'ordine: il gateway deve aspettare che mosquitto sia sano prima di partire. La coppia service/timer di Prophet è invece un pattern standard systemd per job schedulati: il `.timer` decide *quando* eseguire, il `.service` (`Type=oneshot`) decide *cosa* eseguire.
...
└── iot-prophet-forecast.timer
    # pianifica l'esecuzione giornaliera del service sopra
```
...
- Non copre la logica applicativa dei singoli servizi (backend, frontend, gateway, ESP32, prophet), solo come vengono avviati/arrestati o schedulati a livello di sistema operativo. Per il funzionamento del job Prophet, vedi [`prophet/README.md`](../prophet/README.md).
...
Contains the systemd units for automatic system startup and for a scheduled job:
...
└── iot-prophet-forecast.timer
    # schedules the daily run of the service above
```
...
- Does not cover the application logic of the individual services (backend, frontend, gateway, ESP32, prophet), only how they are started/stopped or scheduled at the operating-system level. For how the Prophet job works, see [`prophet/README.md`](../prophet/README.md).